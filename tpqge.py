...
#
# Larger parts of the docstring texts are taken
...
class tpqge(object):
    """ A Python wrapper class for the Gemini API """

    CAN_NOT_FIND_CREDENTIALS = 'Can not find Gemini credentials. Provide the \
    gemini key and secret key either as parameter of tpqge() or in a config \
    file given by the config_file parameter of tpqge().'

    def __init__(self, key="", secret_key="", config_file="", sandbox=True):
        """ Creates a tpqge object, providing methods for all https requests
            in the Gemini API
        ...
        if key != "" and secret_key != "":
            ...
            logging.info('Found key and secret_key as parameter')
        elif config_file != "":
            self.key = tc.get_config_value(config_file, 'gemini', 'key')
            self.secret_key = tc.get_config_value(config_file, 'gemini',
                                                  'secret_key')
            logging.info('Found key and secret_key in additional config file')
        else:
            logging.error('Can not find Gemini log in credentials!')
            raise ValueError(self.CAN_NOT_FIND_CREDENTIALS)

        if sandbox:
            self.url = 'https://api.sandbox.gemini.com/v1/'
            self.socket_url = 'wss://api.sandbox.gemini.com/v1/'
        else:
            self.url = 'https://api.gemini.com/v1/'
            self.socket_url = 'wss://api.gemini.com/v1/'

        logger.info('Url set to %s' % self.url)
        self.last_order_id = None

        self.available_symbols = self.__send_public_request__('symbols')

        self.books_ask = dict()
        self.books_bid = dict()

        for symbol in self.available_symbols:
            self.books_ask[symbol] = dict()
            self.books_bid[symbol] = dict()

    def __send_public_request__(self, method, **kwargs):
        """ Send all public request to the Gemini server. """
        ...
        logger.debug('Sending public request to %s' % url)
        try:
            answer = requests.get(url, timeout=10).json()
        except:
            logger.error('Can not connect to Gemini server')
            raise

        if 'result' in answer and answer['result'] == 'error':
            if 'message' in answer:
                msg = answer['message']
            else:
                msg = 'Unknown'
            logger.error('Gemini server returns an error: %s' % msg)
            raise ServerError('Server returns an error: %s' % msg)
        else:
            logger.debug('Answer: %s' % answer)

        return answer

    def __send_privat_request__(self, method, payload):
        ...
        logger.debug('Sending private request to %s' % url)
        try:
            answer = requests.post(url, headers=headers, timeout=10).json()
        except:
            logger.error('Can not connect to Gemini server')
            raise
    ...
    def new_order(self, symbol, amount, price, side, option='',
        ...
        Will be the same fields as returned by get_order_status()
        """
        ...
        if option and option not in ['maker-or-cancel',
                                     'immediate-or-cancel',
                                     'auction-only']:
            msg = 'option must either be empty or one of "maker-or-cancel",'
            msg += '"immediate-or-cancel", "auction-only"'
        ...
        res = self.__send_privat_request__(method, params)
        return res
    ...
    def get_past_trades(self, symbol, limit_trades=50, since=None):
        ...
        if since is None:
            since = self.make_timestamp(dt.datetime(2000, 1, 1))
            ...
            try:
                ...
                msg = 'since must be of type datetime.date, datetime.datetime '
                msg += 'or an integer'
                raise TypeError(msg)
    ...
    def get_symbols(self):
        """ Return a list of all available symbols for trading. """
        return self.__send_public_request__('symbols')
    ...
    def get_ticker(self, symbol):
        ...
        return self.__send_public_request__(method, **kwargs)
    ...
    def get_current_order_book(self, symbol, limit_bids=50, limit_asks=50):
        ...
        Returns:
        The response will be a dictionary of two dictionaries:
        ...
        try:
            ...
            raise TypeError('limit_bids must be an integer')
        try:
            limit_asks = int(limit_asks)
        except:
            raise TypeError('limit_asks must be an integer')
        return self.__send_public_request__(method, limit_bids=limit_bids,
    ...
    def get_trades_history(self, symbol, since=None, limit_trades=50,
        ...
        data = self.__send_public_request__(method, since=since,
    ...
    def get_current_auction(self, symbol):
        ...
        return self.__send_public_request__(method)
    ...
    def get_auction_history(self, symbol, since=None, limit_auction_results=50,
        ...
        if isinstance(since, dt.date) or isinstance(since, dt.datetime):
            since = self.make_timestamp(since)
        else:
            try:
                since = int(since)
            except:
                msg = 'since must be of type datetime.date, datatime.datetime '
                msg += 'or an integer'
                raise TypeError(msg)

        return self.__send_public_request__(
            method,
            since=since,
    ...
    def make_order_status_streamer(self):
        """ Create and return an instance of the order_status_stream. """
        logger.info('Creating order status stream')
        return order_status_stream(self.socket_url, self.key, self.secret_key)

    def make_market_data_streamer(self, symbol):
        """ Create and return an instance of the market_data_stream for a given 
        symbol.

        Arguments:

        symbol (string):  The symbol to retrieve data for.
        """

        if symbol not in self.available_symbols:
            logger.error('Unknown symbol in make_market_data_stream: %s'
                         % symbol)
            raise ValueError('symbol must be one of %s'
                             % str(self.available_symbols))
        url = self.socket_url + 'marketdata/' + symbol
        logger.info('Creating market data stream for symbol %s' % symbol)
        return market_data_stream(url, self.books_ask[symbol],
                                  self.books_bid[symbol])

    def get_order_book(self, symbol, side):
        """ Return the order for a given symbol.
        ...
        Returns: The order book as dictionary.
        """

        if symbol not in self.available_symbols:
            logger.error('Unknown symbol in get_oder_book: %s' % symbol)
            raise ValueError('symbol must be one of %s'
                             % str(self.available_symbols))
        if side not in ['bid', 'ask']:
            logger.error('Unknown side in get_oder_book: %s' % side)
            raise ValueError("side must be 'bid' or 'ask'")
        if side == 'bid':
            return self.books_bid[symbol]
        else:
            return self.books_ask[symbol]
    ...
    def make_timestamp(self, date):
        ...
        if isinstance(date, dt.date) and not isinstance(date, dt.datetime):
            ...
            logger.error('Mishaped date in make_timestamp: %s' % str(date))
            raise TypeError('date must be of type datetime.date \
                             or datetime.datetime')
        return int(delta.total_seconds() * 1000)


class order_status_stream(object):
    """ Class to implement the order status stream of Gemini. Do not instantiate
        manually, use tpqge.make_order_status_streamer().
    """

    def __init__(self, url, key, secret_key):
        """Creates an order status stream object."""
        self.key = key
        self.secret_key = secret_key
        self.url = url + 'order/events'
        self.ws = None
        self.wst = None
        ...
        self.on_cancel_rejected = None
        self.on_closed = None
        self.on_heartbeat = None
        self.on_subscription_ack = None

    def start_streaming(self):
        """Start the stream"""
        payload = dict()
        payload['request'] = '/v1/order/events'
        payload['nonce'] = int(time.time() * 100000)
        payload = str(json.dumps(payload))

        payload_encode = base64.b64encode(bytearray(payload, 'utf-8'))
        sig = hmac.new(bytearray(self.secret_key, 'utf-8'),
                       payload_encode, hashlib.sha384).hexdigest()
        headers = {'X-GEMINI-APIKEY: %s' % self.key,
                   'X-GEMINI-PAYLOAD:%s' % str(payload_encode)[2:-1],
                   'X-GEMINI-SIGNATURE:%s' % sig}
    ...
    def is_alive(self):
        """ Return True if the stream is connected and False else. """
        return self.wst.is_alive()

    def on_message(self, msg):
        """ Callback method for incoming messages of the stream. """
        try:
            data = json.loads(msg)
        except:
            raise IOError('Can not decode answer.')

        if type(data) == dict:
            if data['type'] == 'heartbeat':
                if self.on_heartbeat is not None:
                    self.on_heartbeat(data)
            elif data['type'] == 'subscription_ack':
                if self.on_subscription_ack is not None:
                    self.on_subscription_ack(data)
            else:
                warnings.warn('Got an unknown event type: %s' % data['type'])
        else:
            for ev in data:
                if ev['type'] == 'initial':
                    if self.on_initial is not None:
                        self.on_initial(ev)
                    ...
                    if self.on_cancelled is not None:
                        self.on_cancelled(ev)
                elif ev['type'] == 'cancel_rejected':
                    if self.on_cancel_rejected is not None:
                        self.on_cancel_rejected(ev)
                elif ev['type'] == 'closed':
                    if self.on_closed is not None:
                        self.on_closed(ev)
                else:
                    warnings.warn('Got an unknown event type: %s' % ev['type'])

    def on_error(self, error):
        """ Callback method for incoming error messages of the stream. """
        print('Error', error)

    def on_open(self):
        """ Callback method if the stream connection is established. """
        logger.info('Opened connection on websocket')

    def on_close(self):
        """ Callback method if the stream connection is closed. """
        logger.info('Connection closed')

    def set_callback(self, event, fkt):
        """ Set user defined callback methods for incoming messages of the stream.

        Arguments:

        event (string): On of 'initial', 'accepted', 'rejected', 'booked',
            'fill', 'cancelled', 'cancel_rejected', 'closed', 'heartbeat' 
            or 'subscription_ack'. Defines the message type for which the 
            callback method should be involved.

        fkt (callable): The method to use as the callback, must accept a 
            dictionary as argument.
        """

        if event not in ['initial', 'accepted', 'rejected', 'booked',
                         'fill', 'cancelled', 'cancel_rejected', 'closed',
                         'heartbeat', 'subscription_ack']:
            msg = "event must be one of  'initial', 'accepted', 'rejected', "
            msg += "'booked', 'fill', 'cancelled', 'cancel_rejected', "
            msg += "'closed', 'heartbeat' or 'subscription_ack'"

            raise ValueError(msg)

        if not callable(fkt):
            raise ValueError('fkt is not callable')
        if event == 'initial':
            self.on_initial = fkt
        elif event == 'accepted':
            self.on_accepted = fkt
        elif event == 'rejected':
            self.on_rejected = fkt
        elif event == 'booked':
            self.on_booked = fkt
        elif event == 'fill':
            self.on_fill = fkt
        elif event == 'cancelled':
            self.on_cancelled = fkt
        elif event == 'cancel_rejected':
            self.on_cancel_rejected = fkt
        elif event == 'closed':
            self.on_closed = fkt
        elif event == 'heartbeat':
            self.on_heartbeat = fkt
        elif event == 'subscription_ack':
            self.on_subscription_ack = fkt

    def unset_callback(self, event):
        """ Unset the callback function bound to the event.
        ...
        event (string): On of 'initial', 'accepted', 'rejected', 'booked',
            'fill', 'cancelled', 'cancel_rejected', 'closed', 'heartbeat'
            or 'subscription_ack'. Defines the message type for which the
            callback method should be unset.
        """
...
class market_data_stream(object):
    """ Class to implement the market data stream of Gemini. Do not instantiate
        manually, use tpqge.make_market_data_streamer().
    """

    def __init__(self, url, book_ask, book_bid):
        """Creates a market data  stream object."""
        self.url = url
        self.ws = None
        self.wst = None
        self.book_ask = book_ask
        self.book_bid = book_bid
        self.on_initial_change = None
        self.on_place_change = None
        self.on_trade_change = None
        self.on_trade = None
        self.on_auction = None
        self.on_change = None
        self.on_cancel_change = None
        logger.info('Initalizing stream object with url %s' % self.url)

    def on_message(self, msg):
        """ Callback method for incoming messages of the stream. """

        logger.debug('Stream %s got a message:' % self.url)
        logger.debug(str(msg))
        try:
            data = json.loads(msg)
        except:
            logger.error('Can not decode stream message to json object')
            ...
            for ev in data['events']:
                if ev['type'] == 'change':
                    if ev['side'] == 'ask':
                        ...
                        book = self.book_bid
                    book[ev['price']] = {'remaining': ev['remaining'],
                                         'delta': ev['delta']}
                    if self.on_change is not None:
                        self.on_change(ev)
                    if ev['reason'] == 'initial':
                        if self.on_initial_change is not None:
                            self.on_initial_change(ev)
                    elif ev['reason'] == 'trade':
                        if self.on_trade_change is not None:
                            self.on_trade_change(ev)
                    elif ev['reason'] == 'place':
                        if self.on_place_change is not None:
                            self.on_place_change(ev)
                    elif ev['reason'] == 'cancel':
                        if self.on_cancel_change is not None:
                            self.on_cancel_change(ev)
                    else:
                        warnings.warn('Got an unknowns change event: %s'
                                      % ev['reason'])

                elif ev['type'] == 'trade':
                    if self.on_trade is not None:
                        self.on_trade(ev)
                elif ev['type'] == 'auction':
    ...
    def start_streaming(self):
        """ Start the stream. """
        logger.info('Starting stream with url %s' % self.url)
        self.ws = websocket.WebSocketApp(self.url, on_message=self.on_message,
                                         on_error=self.on_error,
                                         on_open=self.on_open,
                                         on_close=self.on_close)
    ...
    def stop_streaming(self):
        """ Stop the stream. """
        logger.info('Closing stream with url %s' % self.url)
        self.ws.close()
    ...
    def set_callback(self, event, fkt):
        ...
        event (string): On of 'change', 'initial_change', 'place_change',
            'trade_change', 'cancel_change', 'trade' or 'auction'.
            Defines the message type for which the callback method should be 
            involved.
        ...
        if event not in ['change', 'initial_change', 'place_change',
                         'trade_change', 'cancel_change', 'trade', 'auction']:
            msg = "event must be one of 'change', 'initial_change', "
            msg += "'place_change', 'trade_change', 'cancel_change', 'trade', "
            msg += "'auction'"
            raise ValueError(msg)
        ...
        if event == 'change':
            self.on_change = fkt
        elif event == 'initial_change':
            self.on_initial_change = fkt
        elif event == 'place_change':
            self.on_place_change = fkt
        elif event == 'trade_change':
            self.on_trade_change = fkt
        elif event == 'cancel_change':
            self.on_cancel_change = fkt
        elif event == 'trade':
            self.on_trade = fkt
        elif event == 'auction':
            self.on_auction = fkt
    ...
    def unset_callback(self, event):
        """ Unset user defined callback methods for incoming messages of the
            stream.
